...
test('should show non-logged in state', async ({ navigation, homepage }) => {
  ...
  await test.step(`should show 'Sign in' link in the header'`, async () => {
    await expect(
      navigation.header.signInLink,
      'Sign in link is displayed'
    ).toHaveText('Sign in');
  });
});
...
test.describe('login as Admin', () => {
  test(
    ...
    { tag: '@smoke' },
    async ({ navigation, loginPage, page }) => {
      ...
      await test.step(`should show Admin page`, async () => {
        ...
        await expect(page, 'User is logged in as Admin').toHaveURL(
          /\/admin\/dashboard/
        );
        await expect(
          navigation.header.signInLink,
        ...
        await expect(
          navigation.header.userMenu,
          'should show user name in user menu'
        ).toHaveText(

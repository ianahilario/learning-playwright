...
export class CartItemComponent {
  ...
  async isCorrectProductData(
    ...
    if (expectedIndex !== undefined) {
      ...
      await expect
        .soft(
          productItem,
          `Product '${productData.name}' is displayed in expected index=${expectedIndex}`
        )
        .toBeVisible(productData.name);
    } else {
      productItem = await this.getProductItemLocator(productData);
      await expect
        .soft(productItem, `Product '${productData.name}' is displayed`)
        .toBeVisible(productData.name);
    }
  ...
  async addItemToCartbyIndex(productIndex?: number) {
    ...
    await expect
      .soft(
        productItem.locator(this.addtoCartButton),
        "'Add to cart' button is no longer displayed"
      )
      .not.toBeVisible();
  }
  ...
  async removeItemToCartbyProduct(
    ...
    if (isRemoveFromCartpage) {
      ...
      await expect
        .soft(
          productItem.locator(this.removetoCartButton),
          "'Remove' button is no longer displayed"
        )
        .not.toBeVisible();
    }
  }

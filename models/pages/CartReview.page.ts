...
export class CartReviewPage{
    ...
    async isCorrectOrderDetails(shoppingCart : ShoppingCart){
        ...
        await expect.soft(this.priceSubTotal, "Correct sub-total price").toHaveText(`Item total: $${shoppingCart.subTotalPrice}`);
        await expect.soft(this.priceTax, "Correct tax price").toHaveText(`Tax: $${shoppingCart.taxAmount.toFixed(2)}`);
        await expect.soft(this.priceTotal, "Correct total price").toHaveText(`Total: $${shoppingCart.totalPrice.toFixed(2)}`);
    }
}
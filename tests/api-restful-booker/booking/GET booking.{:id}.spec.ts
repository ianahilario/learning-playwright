...
import { test } from '../../../fixtures/api-restful-booking';
...
test('able to get return booking details', async ({ request }) => {
  ...
  expect.soft(responseBody.bookingdates).toHaveProperty('checkin', '2019-07-11');
  expect.soft(responseBody.bookingdates).toHaveProperty('checkout', '2022-02-06');
...
test('able to handle non-existing booking id', async ({ request }) => {
  const response = await request.get(`https://restful-booker.herokuapp.com/booking/abc`);

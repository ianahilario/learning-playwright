...
export class CartPage extends BasePage {
  ...
  constructor(page: Page, isMobile: boolean) {
    ...
    this.proceedToBillingButton = this.page.locator('//*[@data-test="proceed-2"]');
    this.proceedToPaymentButton = this.page.locator('//*[@data-test="proceed-3"]');
  ...
  async gotoSignInStep() {
    ...
    await this.page
      .locator('//*[contains(@class,"steps-indicator")]//li[contains(@class, "current")]')
      .getByText('Sign in')
  ...
  async selectPaymentMethod(
    paymentMethod: 'bank-transfer' | 'cash-on-delivery' | 'credit-card' | 'buy-now-pay-later' | 'gift-card'
  ) {
  ...
  async gotoOrderConfirmationPage() {
    ...
    await this.page.getByText('Payment was successful').waitFor({ timeout: 5_000 });
    await this.confirmButton.click();
    await this.page.waitForTimeout(1_000);
    await this.page.locator('//*[@id="order-confirmation"]').waitFor({ timeout: 10_000 });
  }

  async getOrderInvoice() {
    const invoiceNumber = (await this.page.locator('//*[@id="order-confirmation"]/span').textContent()) as string;

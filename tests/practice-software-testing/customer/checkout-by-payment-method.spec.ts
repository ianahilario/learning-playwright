...
test(
  ...
  async ({ navigation, homepage, productDetailsPage, cartPage, page }) => {
    await test.step(`should have an empty cart item`, async () => {
      await expect(await navigation.header.getCartQuantity(), 'Cart quantity indicator is not displayed').toBe(
        cartQuantity
      );
    });
    ...
    await test.step(`add an item to cart`, async () => {
      ...
      await expect.soft(productDetailsPage.toastMessage).toHaveText('Product added to shopping cart.');
      cartQuantity++;
      await expect(await navigation.header.getCartQuantity(), `Cart quantity is ${cartQuantity}`).toBe(cartQuantity);
    });
    ...
    await test.step(`proceed to checkout`, async () => {
      ...
      await cartPage.billingAddressForm.addressField.fill(faker.location.streetAddress());
      ...
      await cartPage.billingAddressForm.countryField.fill(faker.location.country());
      await cartPage.billingAddressForm.postodeField.fill(faker.number.int({ min: 10000, max: 99999 }).toString());
    });
    ...
    await test.step(`select payment option and submit`, async () => {
      ...
      await cartPage.bankTransferForm.accountNameField.fill(faker.finance.accountName());
      await cartPage.bankTransferForm.accountNumberField.fill(faker.finance.accountNumber());
    });
    ...
    await test.step(`confirm the order`, async () => {
      ...
      cartQuantity--;
      await expect
        .soft(await navigation.header.getCartQuantity(), `Cart quantity is ${cartQuantity}`)
        .toBe(cartQuantity);
    });
    ...
    await test.step(`should show generated invoice in user's invoice list`, async () => {
      ...
      await expect(page.getByText(invoiceNumber), "Invoice is displayed in user's invoice list").toBeVisible();
    });
  }
);
